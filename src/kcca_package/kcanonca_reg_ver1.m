function [nalpha, nbeta, r] = kcanonca_reg_ver1(Kx,Ky,eta,kapa,sl,nor,Rx,Ry)
...
%
% Usage:
%   [nalpha, nbeta,  r] =  kcanonca_reg_ver1(Kx,Ky,eta,kapa)
%   [nalpha, nbeta,  r] =  kcanonca_reg_ver1(Kx,Ky,eta,kapa,sl)
%   [nalpha, nbeta,  r] =  kcanonca_reg_ver1(Kx,Ky,eta,kapa,sl,nor)
%   [nalpha, nbeta,  r] =  kcanonca_reg_ver1(Kx,Ky,eta,kapa,sl,nor,Rx,Ry)
%
% Input: 
%   Kx, Ky - Kernel matrices corresponding to the two views,
%   eta    - precision parameter for gsd
%   kapa   - regularsation parameter (value 0 to Inf)  
...
%   r             - is a  vector with corresponding canonical correlations.
%
% Non commercial use.
...
S = chol(B)';
invS = inv(S);
A = invS*Zxy*inv(Zyy+kapa*tEyeY)*Zyx*invS';
...
beta = inv(Zyy+kapa*tEyeY)*Zyx*alpha;
t = size(Zyy,1);
